...
import type { Show } from "@/lib/exhibitions";
...
export function ExhibitionBoard() {
  ...
  useEffect(() => {
    fetch(withBase("/data/exhibitions.json"))
      .then((r) => {
      ...
      .then((d) => {
        if (Array.isArray(d.shows)) {
          setShows(d.shows);
          const when = d.fetchedAt ? new Date(d.fetchedAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }) : "";
          setNote(when ? `Updated ${when}` : "");
        }
      })
      .catch(() => setNote("Could not load the live list."));
  }, []);
  ...
  return (
    <div className="mt-10">
      ...
      {note && <p className="mt-4 text-xs uppercase tracking-widest text-ink/40">{note}</p>}
      <ul className="mt-6 space-y-6">
        {visible.map((s) => (
          <li key={s.title + s.venue} className="border-b border-ink/10 pb-6">
            ...
            {s.why && <p className="mt-2 max-w-2xl text-ink/75">{s.why}</p>}
          </li>
        ))}
      </ul>
    </div>
