...
export function SearchDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-ink/40 p-3 pt-[10vh] sm:p-4 sm:pt-[12vh]" onClick={onClose}>
      <div
      ...
      >
        <input
          ...
          className="w-full border-b border-ink/10 bg-transparent px-4 py-3 outline-none sm:py-4"
        />
        <ul className="max-h-[55svh] overflow-auto py-2">
          ...
          {hits.map((h) => (
            <li key={h.type + h.slug}>
              <button
                type="button"
                className="flex w-full flex-col items-start gap-0.5 px-4 py-2 text-left hover:bg-paper-deep sm:flex-row sm:items-baseline sm:justify-between sm:gap-4"
                onClick={() => {
                  router.push(h.href);
                  onClose();
                }}
              >
                <span className="min-w-0">
                  <span className="display text-base sm:text-lg">{h.title}</span>
